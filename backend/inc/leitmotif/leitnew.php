...
if ( ( isset( $_POST[ 'htmlsrc' ] )))
{
	...
	if( $flag === 1)
	{
		...
		{			
			foreach($_FILES as $f)
			{
				if($f['error']==0)
				{
					...
					if(isset($s['mime']))
					{
						...
						{
							$iext = explode('/',$m);	
							$unqid = $hlp->getunqid( $lDesc );
							$imgname = $hlp->getunqid( $unqid );
							$imgDir = "user/leitmotif/$imgname" ;
							$cmd1="mkdir $imgDir";
							exec ( $cmd1, $outa, $ret );
							...
							if( move_uploaded_file( $f['tmp_name'], $tmpPath ))
							{
								...
								{
									//for orde id
									$q1="select count(*)  cnt, max(order_no) max_count from leitmotif;";
									...
									// subgroup
									$q ="insert into leitmotif (lId, lImgId, lDesc,		
									 lCreatedate, lModifydate,order_no) values ('$lId', '$imgname',  
									 '$lDesc', now(), now(),$max);";
									if( ( $res = $hlp->_db->db_query( $q ) ) !== false )
									{
										$hlp->echo_ok( "Leitmotif has been stored." );
									...
									$hlp->echo_err( "Unable to process request. contact support." );	
							}
							else
			...
			$hlp->echo_err("Please upload the neccessary Leitmotif image");	 //img empty
